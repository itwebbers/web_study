...
router.post('/openValue', function (req, res, next) {
    // 判断用户是否传入参数
    try {
        if (req.body && JSON.stringify(req.body) !== '{}' && req.body.userCode.trim() !== '' && req.body.taskNum === 3) {
            console.log(req.body)
            // let flags = 0 // 开阀接口请求是否成功
                                ...
                                try {
                                    request({
                                        ...
                                        result => {
                                            ...
                                            if (result.data && result.data[0].CODE === 'SUCCESS') {
                                                // res.json({ "resultCode": 0, "msg": "操作成功" })
                                                controlBody.exeMark = 2
                                                CommandResult(controlBody, res, { "resultCode": 0, "msg": "操作成功" })
                                            } else {
                                                // res.json({ "resultCode": 1, "msg": "终端设备离线" })
                                                controlBody.exeMark = 3
                                                CommandResult(controlBody, res, { "resultCode": 1, "msg": "终端设备离线" })
                                            }
                                        },
                                        error => {
                                            // res.json(error)
                                            controlBody.exeMark = 1
                                            CommandResult(controlBody, res, error)
                                        }
                                    )
                                } catch (error) {
                                    // res.json(error)
